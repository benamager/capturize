...
export default function useRegister() {
  const { response, error, loading, request } = useAxios(`${API_URL}api/auth/local/register`);
  ...
  useEffect(() => {
    if (error) {
      if (error.response) {
        ...
        if (statusCode === 400) {
          Alert.alert(`Fejlkode: ${statusCode}`, `Enten brugernavn eller email-addresse findes allerede.`)
          return
        }
